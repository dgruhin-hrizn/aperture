...
export async function syncSeriesWatchHistoryForUser(
  ...
  logger.info({ userId, providerUserId }, 'Syncing series watch history')

  const watchedEpisodes = await provider.getSeriesWatchHistory(apiKey, providerUserId)
  ...
  for (const item of watchedEpisodes) {
    ...
    await query(
      `INSERT INTO watch_history (user_id, episode_id, media_type, play_count, last_played_at, is_favorite)
       ...
       ON CONFLICT (user_id, COALESCE(movie_id, '00000000-0000-0000-0000-000000000000'), COALESCE(episode_id, '00000000-0000-0000-0000-000000000000')) DO UPDATE SET
  ...
  logger.info({ userId, synced, removed }, 'Series watch history sync completed')
  return { synced, removed }
}

/**
 * Sync series watch history for all enabled users
 */
export async function syncSeriesWatchHistoryForAllUsers(
  ...
  const jobId = existingJobId || randomUUID()
  createJobProgress(jobId, 'sync-series-watch-history', 2)
  ...
  try {
    ...
    for (let i = 0; i < users.length; i++) {
      ...
      try {
        ...
        const syncResult = await syncSeriesWatchHistoryForUser(user.id, user.provider_user_id)
        totalItems += syncResult.synced
        success++
        addLog(
          jobId,
          'info',
          `✅ ${user.username}: ${syncResult.synced} watched episodes synced`
        )
      } catch (err) {
        const error = err instanceof Error ? err.message : 'Unknown error'
