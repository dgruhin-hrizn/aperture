...
const imageRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post<{ Params: ImageParams; Querystring: ImageQuerystring }>(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        // Get image dimensions (basic approach - could use sharp for more accuracy)
        let width: number | undefined
        let height: number | undefined

        // Try to parse dimensions from buffer (works for PNG/JPEG)
        try {
        ...
        // Sync to media server in background
  ...
  fastify.delete<{ Params: ImageParams; Querystring: ImageQuerystring }>(
    ...
    async (request, reply) => {
      ...
      // Sync the default image back to media server
      if (defaultImage) {
  ...
  fastify.post<{ Params: ImageParams; Querystring: ImageQuerystring }>(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        // Sync default to media server in background
        syncEntityImageToMediaServer(entityType as EntityType, entityId, imageType).catch((err) =>
   ...
   * Downloads the current image from the media server and saves it to Aperture
   */
  fastify.post<{ Params: ImageParams; Querystring: ImageQuerystring }>(
   ...
   */
  fastify.get('/api/images/dimensions', async () => {
