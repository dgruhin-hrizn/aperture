...
- **Usage tracking** — `last_used_at` timestamp for security monitoring
...
## 📺 Seerr: Season Selection for TV Series

When requesting TV series through Seerr, you can now **select specific seasons** instead of requesting the entire show.
...
1. **Shared enrichment** — TMDb data is fetched once and stored in a shared pool
2. **User-specific scoring** — Each user's candidates are scored against their taste profile
3. **Reduced API calls** — Eliminates duplicate TMDb requests across users
...
- `feat: add Seerr season selection modal for TV series requests`
