...
### Watch History Sync Before Recommendations

The recommender now **syncs watch history from your media server** immediately before generating recommendations. This ensures recently watched movies/series are properly excluded (when "include watched" is disabled).

Previously, if you watched something between syncs, it could still appear in recommendations. Now watch history is always up-to-date when generating recommendations.
