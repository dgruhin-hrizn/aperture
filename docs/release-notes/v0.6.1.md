...
This release brings **expanded Top Picks sources**, **TMDB integration for popularity rankings**, **language filtering**, **auto-request support for Seerr**, and several important bug fixes.
...
When requesting missing series through Seerr, you now get **season-level control**:
...
**New Seerr integration for Top Picks!**

When enabled, Aperture can automatically request missing Top Picks items through Seerr:
...
3. Requests are submitted to Seerr automatically
...
- `feat(top-picks): add auto-request integration with Seerr`
- `feat(top-picks): add language filters for movies and series`
- `feat(top-picks): add series season selection for Seerr requests`
...
**Database Migrations:**
- `0099_top_picks_multi_source.sql` — Adds new popularity source columns and auto-request settings
- `0100_top_picks_language_filter.sql` — Adds language filter columns for movies and series
...
1. Navigate to **Settings → Top Picks**
2. Enable auto-request for movies and/or series
3. Configure request limits
4. Ensure Seerr is configured in **Settings → Integrations**
